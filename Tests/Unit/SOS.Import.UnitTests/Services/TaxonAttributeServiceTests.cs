...
namespace SOS.Import.UnitTests.Services
{
    public class TaxonAttributeServiceTests
    {
        ...
        public TaxonAttributeServiceTests()
        {
            ...
            _taxonAttributeServiceConfiguration = new TaxonAttributeServiceConfiguration { BaseAddress = "https://taxonattribute-service.artdata.alu.se" };
        ...
        public void ConstructorTest()
        {
            ...
            create = () => new TaxonAttributeService(
                ...
                null,
                _loggerMock.Object);
            create.Should().Throw<ArgumentNullException>().And.ParamName.Should().Be("taxonAttributeServiceConfiguration");
        ...
        public async Task GetCurrentRedlistPeriodIdAsyncSuccess()
        {
            ...
            //-----------------------------------------------------------------------------------------------------------
            var redListId = 3;
            var redlistPeriod = Newtonsoft.Json.JsonConvert.DeserializeObject(Newtonsoft.Json.JsonConvert.SerializeObject(new { id = redListId }));

            _httpClientServiceMock.Setup(ts => ts.GetDataAsync<dynamic>(It.IsAny<Uri>(), It.IsAny<Dictionary<string, string>>()))
        ...
        public async Task GetCurrentRedlistPeriodIdAsyncFail()
        {
            ...
            //-----------------------------------------------------------------------------------------------------------
            var redListId = 3;
            var redlistPeriod = Newtonsoft.Json.JsonConvert.DeserializeObject(Newtonsoft.Json.JsonConvert.SerializeObject(new { id = 0 }));
        ...
        public async Task GetGetTaxonAttributesAsyncSuccess()
        {
            ...
            // Arrange
            //-----------------------------------------------------------------------------------------------------------
            _httpClientServiceMock.Setup(ts => ts.PostDataAsync<IEnumerable<TaxonAttributeModel>>(It.IsAny<Uri>(), It.IsAny<object>(), It.IsAny<Dictionary<string, string>>()))
                .ReturnsAsync(new []
                {
