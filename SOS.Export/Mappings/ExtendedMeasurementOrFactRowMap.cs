...
namespace SOS.Export.Mappings
{
    ...
    public class ExtendedMeasurementOrFactRowMap : ClassMap<ExtendedMeasurementOrFactRow>
    {
        ...
        public ExtendedMeasurementOrFactRowMap()
        {
            ...
            Map(m => m.MeasurementType).Index(2).Name("measurementType").TypeConverter<LineBreakTabStringConverter<string>>();
            Map(m => m.MeasurementTypeID).Index(3).Name("measurementTypeID");
            Map(m => m.MeasurementValue).Index(4).Name("measurementValue").TypeConverter<LineBreakTabStringConverter<string>>();
            Map(m => m.MeasurementValueID).Index(5).Name("measurementValueID");
            Map(m => m.MeasurementAccuracy).Index(6).Name("measurementAccuracy");
            Map(m => m.MeasurementUnit).Index(7).Name("measurementUnit").TypeConverter<LineBreakTabStringConverter<string>>();
            ...
            Map(m => m.MeasurementRemarks).Index(11).Name("measurementRemarks").TypeConverter<LineBreakTabStringConverter<string>>();
            Map(m => m.MeasurementMethod).Index(12).Name("measurementMethod").TypeConverter<LineBreakTabStringConverter<string>>();
        }
    }
}