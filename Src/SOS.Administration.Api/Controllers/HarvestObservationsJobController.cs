...
namespace SOS.Administration.Api.Controllers
{
    ...
    public class HarvestObservationsJobController : ControllerBase, IHarvestObservationJobController
    {
        ...
        [ProducesResponseType(typeof(string), (int) HttpStatusCode.OK)]
        [ProducesResponseType((int) HttpStatusCode.InternalServerError)]
        public IActionResult AddDailyArtportalenHarvestJob([FromQuery] JobRunModes mode, [FromQuery] int hour, [FromQuery] int minute)
        {
            try
            {
                RecurringJob.AddOrUpdate<IArtportalenHarvestJob>(nameof(IArtportalenHarvestJob),
                    job => job.RunAsync(mode, JobCancellationToken.Null), $"0 {minute} {hour} * * ?", TimeZoneInfo.Local);
        ...
        [ProducesResponseType(typeof(string), (int) HttpStatusCode.OK)]
        [ProducesResponseType((int) HttpStatusCode.InternalServerError)]
        public IActionResult RunArtportalenHarvestJob([FromQuery]JobRunModes mode)
        {
            try
            {
                BackgroundJob.Enqueue<IArtportalenHarvestJob>(job => job.RunAsync(mode, JobCancellationToken.Null));
