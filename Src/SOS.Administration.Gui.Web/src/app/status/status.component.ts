...
export class StatusComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.http.get<ActiveInstanceInfo>(this.baseUrl + 'statusinfo/activeinstance').subscribe(result => {
      ...
      this.http.get<ProcessInfo[]>(this.baseUrl + 'statusinfo/process').subscribe(result => {
        ...
        var activeExpireDate = subHours(new Date(), window.env.lastHarvestHourLimit);
        ...
        var inActiveExpireDate = subHours(new Date(), window.env.lastHarvestHourLimit * 2);
  ...
  private runTests() {
    ...
    this.http.get<FunctionalTest[]>(this.baseUrl + 'tests').subscribe(result => {
      ...
      for (let test of result) {
        this.http.get<TestResults>(this.baseUrl + 'tests/' + test.route).subscribe(result => {
          if (result) {
