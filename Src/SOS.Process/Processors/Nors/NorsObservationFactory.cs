...
namespace SOS.Process.Processors.Nors
{
    public class NorsObservationFactory
    {
        ...
        /// <returns></returns>
        public IEnumerable<ProcessedObservation> CreateProcessedObservations(IEnumerable<NorsObservationVerbatim> verbatims)
        {
            return verbatims.Select(CreateProcessedObservation);
        }

        /// <summary>
        /// Cast NORS observation verbatim to ProcessedObservation
        /// </summary>
        ...
        public ProcessedObservation CreateProcessedObservation(NorsObservationVerbatim verbatim)
        {
            ...
            var obs = new ProcessedObservation(ObservationProvider.NORS)
            {
                BasisOfRecordId = new ProcessedFieldMapValue { Id = (int)BasisOfRecordId.HumanObservation },
                DatasetId = $"urn:lsid:swedishlifewatch.se:dataprovider:{ ObservationProvider.NORS.ToString() }",
                DatasetName = "NORS",
