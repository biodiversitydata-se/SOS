    ...
    this.http.get<FunctionalTest[]>(this.baseUrl + 'tests').subscribe(tests => {
      ...
      for (let test of tests) {
        this.http.get<PerformanceData[]>(this.baseUrl + 'performance/' + test.id).subscribe(result => {
          ...
          this.lineChartData.push({ data: result.map(p => { return { x: Date.parse(p.timestamp), y: p.timeTakenMs } }), label: test.description, backgroundColor: this.idxToColor(test.id) });
        })
      }
    })
  }
