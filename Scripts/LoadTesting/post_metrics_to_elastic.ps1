...
foreach($line in $lines){    
    ...
    Invoke-WebRequest -UseBasicParsing -Uri 'https://artsearch2-1-test.artdata.slu.se:9200/sos-st-loadtests-metrics/_doc?pretty&refresh' -Headers $headers -ContentType "application/json" -Method Post -Body $line | Out-Null
    $index++;
}